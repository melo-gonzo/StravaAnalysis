...
class Strava:
    ...
    def get_access_token(self, override=False):
        # if self.do_download or override==True:
        code = self.strava_code
        access_token = self.client.exchange_code_for_token(
            client_id=self.payload['client_id'], client_secret=self.payload['client_secret'], code=code)
        ...
        if time.time() > access_token['expires_at']:
            ...
            refresh_response = self.client.refresh_access_token(
                client_id=self.payload['client_id'], client_secret=self.payload['client_secret'], refresh_token=access_token['refresh_token'])
    ...
    def year_histogram(self, data, min=4, max=12, bins_per_div=6, activity_type='Run', metric='pace', weight_unit='mile', use_weights=True):
        ...
        plt.hist(data[metric], bins=bins, label=label_mean,
                 color='k', edgecolor='w', weights=weights)
        plt.legend()
        ...
        for idx, year in enumerate(years):
            ...
            ax1.hist(sub[metric], bins=bins, label=str(year) + ' ' +
                     label_mean, color='k', edgecolor='w', weights=weights)
            ax2.hist(sub[metric], bins=bins, alpha=alpha[idx], label=str(
                year) + ' ' + label_mean, edgecolor='w', weights=weights)
            ax1.set_title('{} {}s: {}'.format(
                year, activity_type, len(sub[metric])))
            ax1.set_xlim([min, max])
            ax1.legend()
            ax1.set_ylabel('# {}'.format(y_label))

            ax1.set_xlabel('{} {}'.format(activity_type, metric))
            fig1.savefig(self.fig_dir +
                         '{}_{}_hist.png'.format(year, activity_type))
...
# data.get_access_token(override=False)
data.download_data()
data.organize_data()
...
data.year_histogram(runs, min=4, max=12, bins_per_div=6,
                    activity_type='Run', metric='pace', weight_unit='None', use_weights=False)
